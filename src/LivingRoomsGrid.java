...
public class LivingRoomsGrid extends JPanel {
  ...
  LivingRoomsTransferHandler picHandler;

  public LivingRoomsGrid() {
    ...
    //drop targets.
    pic7 = new DTLivingRooms(null);
    pic7.setTransferHandler(picHandler);
    mugshots.add(pic7);
    pic8 = new DTLivingRooms(null);
    pic8.setTransferHandler(picHandler);
    mugshots.add(pic8);
    pic9 = new DTLivingRooms(null);
    ...
    color = new JSlider();
    mugshots.add(color);
    //pic10 = new DTLivingRooms(null);
    //pic10.setTransferHandler(picHandler);
    //mugshots.add(pic10);
    //pic11 = new DTLivingRooms(null);
    //pic11.setTransferHandler(picHandler);
    //mugshots.add(pic11);
    //pic12 = new DTLivingRooms(null);
  ...
  protected static ImageIcon createImageIcon(String path, String description) {
    java.net.URL imageURL = LivingRooms.class.getResource(path);
  ...
  public static void createAndShowGUI() {
    ...
    JFrame frame = new JFrame("LivingRooms");
    frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

    //Create and set up the menu bar and content pane.
    LivingRoomsGrid demo = new LivingRoomsGrid();
...
class LivingRoomsTransferHandler extends TransferHandler {
  DataFlavor pictureFlavor = DataFlavor.imageFlavor;

  DTLivingRooms sourcePic;
  ...
  public boolean importData(JComponent c, Transferable t) {
    ...
    if (canImport(c, t.getTransferDataFlavors())) {
      DTLivingRooms pic = (DTLivingRooms) c;
  ...
  protected Transferable createTransferable(JComponent c) {
    sourcePic = (DTLivingRooms) c;
    shouldRemove = true;
    return new LivingRoomsTransferHandler.LivingRoomsTransferable(sourcePic);
  }
  ...
  class LivingRoomsTransferable implements Transferable {
    private Image image;

    LivingRoomsTransferable(DTLivingRooms pic) {
      image = pic.image;
    }
...
/*
 * DTLivingRooms.java is used by the 1.4 LivingRooms.java example.
 */

//A subclass of LivingRooms that supports Data Transfer.

class DTLivingRooms extends LivingRooms implements MouseMotionListener {
  private MouseEvent firstMouseEvent = null;

  private static boolean installInputMapBindings = true;

  public DTLivingRooms(Image image) {
    super(image);
    ...
    //LivingRooms does not use menu accelerators and, since
    //the default value of installInputMapBindings is true,
    //the bindings are installed. LivingRooms2 does use
  ...
  //This method is necessary because LivingRooms and
  //LivingRooms2 both use this class and LivingRooms
  //needs to have the input map bindings installed for
  //cut/copy/paste. LivingRooms2 uses menu accelerators
...
/*
 * LivingRooms.java is used by the 1.4 TrackFocusDemo.java and LivingRooms.java
 * examples.
 */

class LivingRooms extends JComponent implements MouseListener, FocusListener,
    Accessible {
  Image image;

  public LivingRooms(Image image) {
